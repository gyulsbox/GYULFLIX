...
const MovieDetail = () => {
  ...
  const { data: info } = useQuery<IGetMoviesDetail>("MovieDetail", () => getMoviesDetail(movieMatch?.params.movieId));
  const { data: similar } = useQuery<IGetMovieSimilar>("MovieSimilar", () => getMovieSimilar(movieMatch?.params.movieId));
  const onClicked = () => {
    navigate("/movies");
  };
                  ...
                  <SemiContainer>
                    ...
                    <SemiBox>
                      {similar?.results.slice(0, 6).map((movie, index) => {
                        return (
                          <>
                            <Similar key={index} variants={similarVars} whileHover="hover" transition={{ type: "tween" }} bgimg={makeImagePath(movie.backdrop_path || "")}>
